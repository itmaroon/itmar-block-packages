...
const createBlockAttr = (selectedField, pluginOption) => {
    ...
    let blockAttributes = {};
    const pluginUrl = window[pluginOption]?.plugin_url || "";
    switch (selectedField.block) {
        ...
        case "itmar/slide-mv":
            ...
            const slideBlock = createBlock("itmar/design-group", {
                default_val: {
                    ...defaultValBase,
                    width_val: "fit",
                },
            ...
            const slideBlocks = Array.from({ length: 5 }, () => createBlockTree(serializedSlide));
            //子ブロック付きで返す
            blockAttributes = {
...
const getBlocksSnapshot = (blocks = []) => JSON.stringify(blocks.map((block) => normalizeBlockSnapshot(serializeBlockTree(block))));
...
selectedFields, pickupType, dispTaxonomies, sectionCount, domType, clientId, insertId, pluginOption, onRebuildBlocks) => {
    ...
    const pickupBlock = useSelect((select) => select("core/block-editor").getBlock(clientId), [clientId]);
    const insertBlock = useSelect((select) => select("core/block-editor").getBlock(insertId), [insertId]);
    useEffect(() => {
        ...
        const blocksArray = normalizedDispAttributeArray.map((dispAttribute, unit_index) => {
            ...
            const userBlocks = Array.isArray(dispAttribute.innerBlocks)
                ? filterBlocksRecursively(dispAttribute.innerBlocks).map(createBlockTree)
                : [];
            // autoGenerated（selectedFields） + userBlocks を合成
            const innerBlocks = [...userBlocks, ...autoGeneratedBlocks];
            const ret = createBlock("itmar/design-group", {
