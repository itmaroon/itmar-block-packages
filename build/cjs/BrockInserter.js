...
var createBlockAttr = selectedField => {
  ...
  switch (selectedField.block) {
    ...
    case "itmar/slide-mv":
      ...
      var slideBlock = blocks.createBlock("itmar/design-group", {
        default_val: _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, defaultValBase), {}, {
          width_val: "fit"
        }),
        mobile_val: _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, mobileValBase), {}, {
          width_val: "fit"
        })
      }, [imageBlock]);
      //slideBlock をシリアライズ
      var serializedSlide = blockStore.serializeBlockTree(slideBlock);
      // 同じスライドブロックを5つ複製（独立したブロックとして）
      var slideBlocks = Array.from({
        length: 5
      }, () => blockStore.createBlockTree(serializedSlide));
      //子ブロック付きで返す
      blockAttributes = {
        attributes: {
...
var useRebuildChangeField = (dispAttributeArray, selectedFields, pickupType, dispTaxonomies, sectionCount, domType, clientId, insertId) => {
  ...
  var pickupBlock = data.useSelect(select => select("core/block-editor").getBlock(clientId), [clientId]);
  element.useEffect(() => {
    ...
    var blocksArray = dispAttributeArray.map((dispAttribute, unit_index) => {
      ...
      var allBlocks = Array.isArray(dispAttribute.innerBlocks) ? blockStore.flattenBlocks(dispAttribute.innerBlocks) //階層になったブロックを平坦化
      : [];
      ...
      var autoGeneratedBlocks = newSelectedFields.filter(field => !existingKeys.includes(field.key)).map(field => {
        ...
        return blocks.createBlock(blockName, blockAttributes, innerBlocks);
      });
      ...
      var filterBlocksRecursively = blocks => {
        return blocks.map(block => {
          ...
          // 構造を復元して返す
          return _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, block), {}, {
            innerBlocks: filteredInner
          });
        }).filter(Boolean); // null を除去
      };
      var userBlocks = Array.isArray(dispAttribute.innerBlocks) ? filterBlocksRecursively(dispAttribute.innerBlocks).map(blockStore.createBlockTree) : [];

      // autoGenerated（selectedFields） + userBlocks を合成
      var innerBlocks = [...userBlocks, ...autoGeneratedBlocks];
      var ret = blocks.createBlock("itmar/design-group", _rollupPluginBabelHelpers.objectSpread2(_rollupPluginBabelHelpers.objectSpread2({}, dispAttribute.attributes), {}, {
