...
const createBlockAttr = (selectedField, pluginOption) => {
    ...
    switch (selectedField.block) {
        ...
        case "itmar/slide-mv":
            ...
            //slideBlock をシリアライズ
            const serializedSlide = blockStore.serializeBlockTree(slideBlock);
            // 同じスライドブロックを5つ複製（独立したブロックとして）
            const slideBlocks = Array.from({ length: 5 }, () => blockStore.createBlockTree(serializedSlide));
            //子ブロック付きで返す
            blockAttributes = {
    ...
    element.useEffect(() => {
        ...
        const blocksArray = dispAttributeArray.map((dispAttribute, unit_index) => {
            ...
            const userBlocks = Array.isArray(dispAttribute.innerBlocks)
                ? filterBlocksRecursively(dispAttribute.innerBlocks).map(blockStore.createBlockTree)
                : [];
            // autoGenerated（selectedFields） + userBlocks を合成
            const innerBlocks = [...userBlocks, ...autoGeneratedBlocks];
            const ret = blocks.createBlock("itmar/design-group", {
