...
export default function MasonryControl(
  ...
  images: MasonryImage[] = [],
  { columns = 1, renderItems = true }: MasonryOptions = {},
): any {
  ...
  if (renderItems) {
    ...
    // sizer がなければ作る
    let sizer = gridEl.querySelector(".itmar-masonry-sizer") as HTMLElement;
    if (!sizer) {
    ...
    gridEl.querySelectorAll(".itmar-masonry-item").forEach((node) => {
      (node as HTMLElement).style.width = `${columnWidthPercent}%`;
    });
  }
