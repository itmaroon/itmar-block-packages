...
const normalizeDispAttributeArray = (items: any[] = [], length: number) => {
  const next = [...items];
...
const normalizeBlockSnapshot = (value: any): any => {
  ...
  const { clientId, isValid, originalContent, validationIssues, ...rest } =
    value;

  return Object.fromEntries(
    Object.entries(rest).map(([key, entryValue]) => [
      key,
...
const getBlocksSnapshot = (blocks: any[] = []) =>
  JSON.stringify(
    blocks.map((block) => normalizeBlockSnapshot(serializeBlockTree(block))),
  );
...
export const useRebuildChangeField = (
  ...
  insertId: string,
  pluginOption: string,
  onRebuildBlocks?: (blocks: any[]) => void,
) => {
  // dispatch関数を取得
  ...
  const insertBlock = useSelect(
    (select) => (select("core/block-editor") as any).getBlock(insertId),
    [insertId],
  );

  useEffect(() => {
    //dispAttributeArray の個数調整
    const normalizedDispAttributeArray = normalizeDispAttributeArray(
      dispAttributeArray,
      sectionCount,
    );

    // インナーブロックに差し込むブロック配列を生成
    const blocksArray = normalizedDispAttributeArray.map(
      (dispAttribute, unit_index) => {
