...
// 1. まず attributes の中身を定義
interface BlockPlaceAttributes {
  positionType: string;
  isPosCenter: boolean;
  default_val: any; // 具体的な構造がわかればここも詳細に定義
  mobile_val: any;
  [key: string]: any; // 他の属性がある場合の保険
}

// 2. コンポーネント全体の Props を定義
...
export default function BlockPlace(props: BlockPlaceProps) {
    ...
    <>
      ...
      >
        ...
        <BlockWidth
          ...
          isMobile={isMobile}
          isSubmenu={isSubmenu}
          onWidthChange={(key: string, widthVal: string | number) =>
            props.onWidthChange(key, widthVal)
          }
          onFreeWidthChange={(key: string, freeVal: string | number) =>
        ...
        <BlockHeight
          attributes={attributes}
          isMobile={isMobile}
          onHeightChange={(heightVal: string) =>
            props.onHeightChange(heightVal)
          }
          onFreeHeightChange={(freeVal: string | number) =>
            props.onFreeHeightChange(freeVal)
          }
        />
...
interface BlockWidthProps {
  attributes: any; // または BlockPlaceAttributes
  isMobile: boolean;
  isSubmenu: boolean;
...
export function BlockWidth(props: BlockWidthProps) {
