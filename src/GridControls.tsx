...
type AlignIconKey = keyof typeof alignIconMap;
...
const initializeUnitArray = (
  rowUnit: string[] | undefined | null,
  length: number,
): string[] => {
...
interface GridElement {
  ...
  endCell: GridCell;
  value?: string;
  [key: string]: any; // その他のプロパティ（attributesなど）を許容
}
const findElementInGrid = (gridElms: GridElement[], r: number, c: number) => {
...
interface StopPropagationWrapperProps {
  children: ReactNode; // JSX、文字列、数値、配列など何でも受け取れる型
}
...
// 1. attributes の詳細な型定義
export interface GridAttributes {
  gridElms: GridElement[]; // 先ほど定義した GridElement の配列
  rowNum: number;
  colNum: number;
  ...
  rowUnit: string[];
  colUnit: string[];
  [key: string]: any; // 他の属性（背景色など）がある場合の保険
}

// 2. コンポーネント全体の Props 定義
interface GridControlsProps {
  attributes: GridAttributes;
  clientId: string;
  // 更新関数。一部の属性だけを渡せるように Partial を使います
...
const GridControls = ({
            ...
            {setElm && (
              <StopPropagationWrapper>
                <ToolbarDropdownMenu
                  ...
                  icon={
                    setElm.elm.latAlign
                      ? alignIconMap[setElm.elm.latAlign as AlignIconKey]
                      : alignIconMap["left"]
                  }
                  controls={["left", "center", "right"].map((align) => ({
                    icon: alignIconMap[align as AlignIconKey],
                    title: __(
                ...
                <ToolbarDropdownMenu
                  ...
                  icon={
                    setElm.elm.vertAlign
                      ? alignIconMap[setElm.elm.vertAlign as AlignIconKey]
                      : alignIconMap["upper"]
                  }
  ...
  //テーブルの位置選択関数
  const detectCellPosition = (rowIndex: number, colIndex: number) => {
  ...
  //コンテンツ位置設定
  const updateAlignment = (index: number, align: string, derection: string) => {
  ...
  //選択したインナーブロック
  interface SelBlockType extends GridElement {
    value: string;
    label: string;
    startCell: { rowInx: number; colInx: number };
    endCell: { rowInx: number; colInx: number };
  }
  const [selBlock, setSelBlock] = useState<SelBlockType | null>(null);
  ...
  const parentBlocks = useSelect(
    (select) => {
      ...
      const new_block_names = innerBlocks.map((block: any, index: number) =>
  ...
  // 1. blockNames が持つべき型を定義（既存の GridElement か、シンプルなラベル/値のペア）
  type BlockNameItem = SelBlockType | { value: string; label: string };
  const [blockNames, setBlockNames] = useState<BlockNameItem[]>(parentBlocks);
  ...
  const [unitRowArray, setUnitRowArray] = useState<string[]>(initRowUnitArray);
  const initColUnitArray = initializeUnitArray(colUnit, colCount);
  const [unitColArray, setUnitColArray] = useState<string[]>(initColUnitArray);

  //親ブロックへの書き戻し
  useEffect(() => {
    const gridStyle: GridAttributes = {
      ...attributes,
      gridElms: blockNames as GridElement[], // BlockNameItem[] を GridElement[] に合わせる
      rowNum: rowCount,
      colNum: colCount,
    ...
    <>
      ...
      <ComboboxControl
        ...
        onChange={(sel_id) => {
          ...
          if (matchedBlock) {
            setSelBlock(matchedBlock as SelBlockType);
          } else {
            setSelBlock(null);
          }
        }}
