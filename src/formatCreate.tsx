...
interface PlainFormatOption {
  label: string;
  key: string;
  value: string | Intl.NumberFormatOptions;
}
...
export const displayFormated = (
  ...
  const isDateFormat = (dateFormats as DateFormatOption[]).find(
    (f) => f.value === userFormat,
  );
  ...
  //数値や文字列のフォーマット
  const selectedFormat = (plaineFormats as PlainFormatOption[]).find(
    (f) => f.key === userFormat,
  )?.value;
  ...
  if (typeof selectedFormat === "object" && selectedFormat !== null) {
    ...
    try {
      ...
      if (isNaN(numeric)) return String(content);
      // `selectedFormat` を元に新しいフォーマット設定を生成（mutateしない）
      // options を型安全に生成
      const options: Intl.NumberFormatOptions = { ...selectedFormat };
