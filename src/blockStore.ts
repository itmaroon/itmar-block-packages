...
export const useTargetBlocks = (
  ...
  const targetblocks = useSelect(
    (select) => {
      ...
      const filtered = (siblingBlocks as BlockInstance[]).filter(
...
//ネストしたブロックを平坦化
export const flattenBlocks = (blocks: BlockInstance[]): BlockInstance[] => {
  return blocks.reduce((acc: BlockInstance[], block: BlockInstance) => {
    acc.push(block);
...
export const serializeBlockTree = (block: BlockInstance): SerializedBlock => {
  return {
...
export const createBlockTree = (blockData: SerializedBlock): BlockInstance => {
