const createRGB = (
  ...
  //１６進数変換の関数
  const componentToHex = (c: string | number): string => {
    const hex = parseInt(String(c), 10).toString(16);
    return hex.length === 1 ? "0" + hex : hex;
  };

  // デフォルト値（白）
  if (!inputStr) return ["ff", "ff", "ff"];
 ...
 */
export function rgb16ToHsl(strRgb16: string): HSL | false {
 ...
 */
export function HexToRGB(strRgb16: string): RGB | false {
