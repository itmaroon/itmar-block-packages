...
= 3.2.3 =
- WordPressをサブディレクトリに設置した環境でも、Shopify認証用REST APIのURLを正しく解決するように修正
- 旧形式の `/wp-json/` 付きルートもサイト固有のRESTルートへ正規化するように改善

= 3.2.2 =
- Shopify OAuthの開始処理をサーバー経由に変更し、認証情報をブラウザーに保存しない構成に改修
- Shopifyのログイン状態をサーバー側のセッションで検証するように変更し、APIエラー内容の取得を改善
...
### restTaxonomies
投稿タイプに登録されているタクソノミー（カテゴリ、タグを含む）の情報およびそのターム情報を取得して配列で返します。公開RESTコンテキストを使用するため、公開されている投稿タイプとタクソノミーであれば、未ログインのフロントエンドからも利用できます。
#### 引数
- `post_type` string  
投稿タイプのスラッグ  
